...
`GetAccount` can return the state of a public account as of a specific past block. The node keeps this history only
for the most recent `50` blocks: vault assets, storage map values, and account code that can no longer affect any block
...
Requesting a block that has fallen outside the window fails with `INVALID_ARGUMENT` and a message stating that the
block has been pruned. This is one of the ordinary-status cases described above: it carries no Miden error detail code,
and the message should not be matched on. The effective boundary may also lag the chain tip by a few blocks while
in-flight reads still pin older snapshots, so treat the window as approximate rather than exact.
