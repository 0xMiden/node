...
Genesis also creates the public wallet `batch_builder.mac`. Pass this file to the sequencer with
`--batch.builder.account`. This account receives the fees collected during batch building. Keep the account file private
because it contains the batch builder signing key.
...
fee notes into one P2ID payment without changing its state. Pass this file to the sequencer with
`--batch.builder.pass-through-account`. Keep the file private because it contains the collection account's signing key.
