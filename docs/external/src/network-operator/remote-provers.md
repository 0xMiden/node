...
Remote provers offload expensive transaction, batch, or block proof generation to dedicated
machines. Each `miden-remote-prover` instance is configured for one proof type.
...
Remote provers are internal services. Put load balancing, service discovery, and admission control
outside the prover binary when a deployment needs them.
