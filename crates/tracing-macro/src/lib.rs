...
pub fn miden_instrument(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let attr_has_fields = attr.to_string().contains("fields");
    let inferred_fields = (!fields.is_empty()).then(|| {
        quote! { fields(#(#fields = ::tracing::field::Empty),*) }
    });
    let args = match (attr.is_empty(), attr_has_fields, inferred_fields) {
        (true, _, Some(fields)) => fields,
        (true, _, None) => quote! {},
        (false, true, _) | (false, false, None) => attr,
        (false, false, Some(fields)) => quote! { #attr, #fields },
    };
