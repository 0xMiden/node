...
impl RocksDbSnapshotInner {
    fn new(db: Arc<DB>) -> Self {
        ...
        // keeps that `Arc<DB>` alive and its `Drop` impl releases the snapshot before the `Arc`
        // is dropped by Rust's normal field cleanup, so the borrow stays valid for the snapshot's
...
impl Drop for RocksDbSnapshotInner {
    fn drop(&mut self) {
        // SAFETY: `snapshot` is only wrapped in `ManuallyDrop` to control field-drop order. We
        // drop it exactly once here, before `db` is dropped by Rust's normal field cleanup.
        // Mirrors the upstream `RocksDbSnapshotInner::drop` in `miden_crypto`.
        unsafe {
