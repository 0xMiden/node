...
`miden-large-smt-backend-rocksdb` provides RocksDB-backed storage for large Sparse Merkle Trees used
by the Miden node store. It is part of the
...
Building this crate requires the native toolchain support needed by RocksDB bindings. The exact
system package names vary by platform, so use the build error output and the node installation docs
