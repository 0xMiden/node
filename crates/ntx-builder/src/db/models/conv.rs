...
pub fn account_from_bytes(bytes: &[u8]) -> Result<Account, DatabaseError> {
    Account::read_from_bytes(bytes)
        .map_err(|e| DatabaseError::InteractError(format!("failed to deserialize account: {e}")))
}

pub fn account_id_from_bytes(bytes: &[u8]) -> Result<AccountId, DatabaseError> {
    AccountId::read_from_bytes(bytes)
        .map_err(|e| DatabaseError::InteractError(format!("failed to deserialize account id: {e}")))
}
...
pub fn network_account_id_from_bytes(bytes: &[u8]) -> Result<NetworkAccountId, DatabaseError> {
    ...
    NetworkAccountId::try_from(account_id).map_err(|e| {
        DatabaseError::InteractError(format!("account id is not a network account: {e}"))
    })
}
...
pub fn single_target_note_from_bytes(
    ...
    let proto_note = proto::note::NetworkNote::decode(bytes).map_err(|e| {
        DatabaseError::InteractError(format!("failed to decode network note proto: {e}"))
    })?;
    SingleTargetNetworkNote::try_from(proto_note).map_err(|e| {
        DatabaseError::InteractError(format!("failed to convert network note from proto: {e}"))
    })
}
