...
impl AccountActor {
    ...
    pub async fn run(mut self, semaphore: Arc<Semaphore>) -> ActorShutdownReason {
        ...
        let has_notes = self
            .db
            .query("initial_available_notes", {
                let max_note_attempts = self.max_note_attempts;
                move |conn| {
                    let notes =
                        queries::available_notes(conn, account_id, block_num, max_note_attempts)?;
                    Ok::<bool, crate::db::errors::DatabaseError>(!notes.is_empty())
                }
            })
            .await
                ...
                permit = tx_permit_acquisition => {
                    match permit {
                        Ok(_permit) => {
                            ...
                            let max_note_attempts = self.max_note_attempts;
                            if let Err(err) = self.db.transact("drop_failing_notes", {
                                move |conn| {
                                    queries::drop_failing_notes(conn, account_id, max_note_attempts)
                                }
                            }).await {
    ...
    async fn select_candidate_from_db(
        ...
        let (latest_account, notes) = self
            ...
            .query("select_candidate", move |conn| {
                ...
                let notes =
                    queries::available_notes(conn, account_id, block_num, max_note_attempts)?;
                Ok::<_, crate::db::errors::DatabaseError>((account, notes))
            })
            .await
