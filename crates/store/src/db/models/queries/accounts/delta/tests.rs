...
/// Regression test: the partial delta path must preserve a fungible asset's callback flag.
///
/// The callback flag is part of the asset's vault key, so dropping it makes the store recompute a
/// different vault root (and account commitment) than the kernel. This surfaces as an
/// `AccountCommitmentsMismatch` on the second delta to the same account: block 2 stores the asset
/// (flag dropped pre-fix), and block 3 reads it back and recomputes a diverging root.
#[test]
