...
fn test_upsert_accounts_updates_is_latest_flag() {
    ...
    let metadata = AccountComponentMetadata::new("test::interface")
        .with_supported_type(AccountType::RegularAccountImmutableCode);
    let component_2 =
        AccountComponent::new(account_component_code, component_storage_modified, metadata)
            .unwrap();
...
fn test_upsert_accounts_with_empty_storage() {
    ...
    let metadata = AccountComponentMetadata::new("test::interface")
        .with_supported_type(AccountType::RegularAccountImmutableCode);
    let component = AccountComponent::new(account_component_code, vec![], metadata).unwrap();
