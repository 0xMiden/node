...
impl InnerForest {
    ...
    fn insert_account_storage(
        ...
        for (slot_name, map_delta) in delta.maps() {
            ...
            // explicitly
            let raw_map_entries: Vec<(StorageMapKey, Word)> = map_delta
                .entries()
                .iter()
            ...
            let hashed_entries: Vec<(Word, Word)> =
                raw_map_entries.iter().map(|(key, value)| (key.hash().into(), *value)).collect();
    ...
    fn update_account_storage(
        ...
        for (slot_name, map_delta) in delta.maps() {
            ...
            let prev_root = self.get_latest_storage_map_root(account_id, slot_name);
            let delta_entries: Vec<(StorageMapKey, Word)> = map_delta
                .entries()
                .iter()
                .map(|(key, value)| (StorageMapKey::new((*key).into()), *value))
                .collect();

            // Hash the keys before inserting into the forest, matching how StorageMap
            // hashes keys before inserting into the SMT.
            let hashed_entries: Vec<(Word, Word)> =
                delta_entries.iter().map(|(key, value)| (key.hash().into(), *value)).collect();
