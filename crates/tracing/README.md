...
All function arguments are skipped automatically. The component name becomes both the tracing
`target` and a prefix for the span name (`component.fn_name`).
...
The component prefix can also be a string literal when no in-scope const is available:
...
`COMPONENT` is either an identifier (referencing a `&str` const in scope) or a string literal.
...
| `err` | On `Err`: emit a tracing event with the top-level error message (delegates to `tracing::instrument`'s built-in `err`). Mutually exclusive with `report`. Requires `Result` return. |
...
use explicit `fields(...)` entries instead.
