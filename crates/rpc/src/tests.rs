...
/// Same as `build_test_proven_tx` but lets the caller supply the `AccountId`.
/// Uses a non-empty `initial_state_commitment` so the result is a post-deployment tx.
...
async fn rpc_rejects_post_deployment_network_account_tx() {
    ...
    // Seed a row marking a known AccountId as a network account directly in the
    // store's SQLite DB. The store uses WAL mode so a secondary connection is safe.
...
// Batch-path coverage for the network-account gate is provided manually.
// Building a valid `ProposedBatch` + `ProvenBatch` in this test harness would
// require duplicating LocalBatchProver setup. The query layer is covered by
// the unit test in store::db::tests, and the gRPC wiring is the same as
// `submit_proven_transaction` (covered by `rpc_rejects_post_deployment_network_account_tx`).
