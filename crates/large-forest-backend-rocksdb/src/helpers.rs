...
pub(crate) fn get_leaf_value(leaf: &SmtLeaf, key: Word) -> Result<Option<Word>, StorageError> {
    match leaf {
        SmtLeaf::Empty(_) => Ok(None),
        SmtLeaf::Single((k, v)) => {
            if *k == key {
                Ok(Some(*v))
            } else {
        ...
        SmtLeaf::Multiple(pairs) => match pairs.binary_search_by(|(k, _v)| {
            LexicographicWord::from(*k).cmp(&LexicographicWord::from(key))
        }) {
            Ok(pos) => Ok(Some(pairs[pos].1)),
            Err(pos) => {
                if pairs.len() >= MAX_LEAF_ENTRIES {
