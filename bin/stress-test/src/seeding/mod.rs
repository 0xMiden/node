...
pub async fn seed_store(
    ...
    let (state, _proven_tip) = State::load_with_database_options(
        ...
        miden_node_store::DatabaseOptions::default(),
        termination_ask,
    )
    .await
...
async fn generate_blocks(
    ...
    for i in 0..total_blocks {
        ...
        // update blocks
        prev_block_header = apply_block(batches, block_inputs, state, &mut metrics, signer).await;
        account_states
...
async fn apply_block(
    ...
    let proving_block_inputs = block_inputs.clone();
    ...
    let proving_inputs = BlockProofRequest {
        ...
        block_inputs: proving_block_inputs,
    };
    state
        .save_proving_inputs(block_header.block_num(), &proving_inputs)
        .await
        .unwrap();
    state.apply_block(signed_block).await.unwrap();
...
async fn get_batch_inputs(
    ...
    let batch_inputs = state
        .get_batch_inputs(
            BTreeSet::from([block_ref.block_num()]),
