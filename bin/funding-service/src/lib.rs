...
//! to any account which asks for it. Each request produces a public pay-to-ID note, which the
//! service answers with at once and then creates on chain.
...
/// How many notes may wait in the queue, per note of one transaction.
...
impl FundingServiceConfig {
    ...
    pub async fn build(self) -> anyhow::Result<FundingService> {
        ...
        // which the node serves with the genesis block header. The remaining fee parameters are in
        // every block header, and the status refresher reads them at the block it reports.
        let protocol_config = node.protocol_config().clone();
        let fee_asset_id = protocol_config.fee_asset_id();
...
impl FundingService {
    ...
    pub async fn run(
        ...
        tasks.spawn("funder", async move {
            Box::pin(funder.run(request_receiver, worker_shutdown))
                .await
