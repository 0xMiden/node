...
impl TopUp {
    ...
    pub async fn run(mut self, shutdown: CancellationToken) -> Result<()> {
        loop {
            ...
            if let Err(err) = Box::pin(self.collect()).await {
                warn!(
                    &err,
                    target: LOG_TARGET,
    ...
    async fn collect(&mut self) -> Result<()> {
        ...
        let total: u64 = deposits
            .iter()
            .map(|note| native_amount(note, self.protocol_config.fee_asset_id()))
            .sum();
        info!(
            target: LOG_TARGET,
    ...
    async fn find_deposits(&mut self) -> Result<Vec<Note>> {
        ...
        let candidates: Vec<Note> = notes
            ...
            .filter(|note| {
                is_deposit(note, self.key.account_id(), self.protocol_config.fee_asset_id())
            })
            .collect();
        ...
        // A rescan sees deposits which an earlier collection already consumed. A transaction which
        // consumes a note twice fails, so the spent notes are dropped here.
    ...
    async fn consume(&mut self, deposits: Vec<Note>) -> Result<()> {
        ...
        let fee_faucet = self
            .node
            .public_account(self.protocol_config.fee_asset_id().faucet_id(), reference_block)
            .await?;
...
fn is_deposit(note: &Note, funder: AccountId, fee_asset_id: AssetId) -> bool {
...
fn native_amount(note: &Note, fee_asset_id: AssetId) -> u64 {
    note.assets()
        .iter()
...
mod tests {
    ...
    fn a_native_asset_note_for_the_funder_is_a_deposit() {
        ...
        assert!(is_deposit(&note, funder, AssetId::new_fungible(funder)));
        assert_eq!(native_amount(&note, AssetId::new_fungible(funder)), 5_000);
    }
    ...
    fn a_note_holding_another_asset_is_skipped() {
        ...
        assert!(!is_deposit(&note, funder, AssetId::new_fungible(funder)));
        assert_eq!(native_amount(&note, AssetId::new_fungible(funder)), 0);
    }
