...
///
/// Produced once per committed block on the ntx-builder side; downstream code (coordinator, DB)
/// applies the contained effects to local state and notifies affected actors.
#[derive(Debug, Clone)]
...
impl CommittedBlockEffects {
    /// Filters the committed block down to the slice the ntx-builder cares about: network notes,
    /// network-account updates, and nullifiers.
    ///
    /// Non-network output notes and non-network account updates are dropped. Private output notes
    /// cannot be network notes (which must be public) and are skipped.
    pub fn from_signed_block(block: &SignedBlock) -> Self {
        ...
        let network_account_updates = body
            ...
            .filter_map(|update| {
                ...
                let network_id = NetworkAccountId::try_from(account_id).ok()?;
